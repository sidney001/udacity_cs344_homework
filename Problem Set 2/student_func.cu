                   ...
                   const float* const filter, const int filterWidth)
{
  ...
  int current_row = thread_2D_pos.y; // center
  ...
  // for (int i=0;i<filterWidth;i++) {
    ...
    // int pi_i = min(numRows-1, max(0, i_i)); // paddedImage_i
    // for (int j=0;j<filterWidth;j++) {
      // int f_j = j - filterCenter;
      // int i_j = c_j + f_j;
      // int pi_j = min(numCols-1, max(0, i_j));
  ...
  for (int filter_row = -filterWidth/2; filter_row <= filterWidth/2; ++filter_row) {
        for (int filter_col = -filterWidth/2; filter_col <= filterWidth/2; ++filter_col) {
			 ...
			 // int image_row = min(max(current_row + filter_row, 0), static_cast<int>(numRows - 1));
             // int image_col = min(max(current_col + filter_col, 0), static_cast<int>(numCols - 1));
			 int image_row = min(max(current_row + filter_row, 0), (numRows - 1));
             int image_col = min(max(current_col + filter_col, 0), (numCols - 1));
                                ...
                                const float* const h_filter, const size_t filterWidth)
{
  ...
  //Use the pointer d_filter that we have already declared for you //device GPU
  ...
  //on the GPU.  cudaMemcpy(dst, src, numBytes, cudaMemcpyHostToDevice); // dst destination
  //Remember to use checkCudaErrors!                                     // src source
...
void cleanup() {
  ...
  checkCudaErrors(cudaFree(d_filter));//TODO
}
// Good job! Your image matched perfectly to the reference image
//Your program ran and executed in 1.627488 ms.
